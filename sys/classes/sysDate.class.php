...
class Date_Calc {
    ...
    /**
    * Class method DateDiffInTime
    * return difference betwen two dates in time
    * @param integer $date1 - first date as timestamp. Must be earlier than $date2
    * @param integer $date2 - second date as timestamp. Must be bigger than $date1
    * @return stinng as time '04:12:43'
    * @author Igor Trokhymchuk  <ihor@seotm.com>
    * @version 1.0, 27.12.2010
    */    
    function DateDiffInTime ($date1,$date2) {
        // получает количество секунд между двумя датами 
        $timedifference = $date2 - $date1;
        
        //$hours = bcdiv($timedifference, 3600);
        $hours = round($timedifference/3600);
        if(strlen($hours)==1) $hours = '0'.$hours;  
        //$minutes = bcdiv(( $timedifference % 3600 ),60);
        $minutes = round(( $timedifference % 3600 )/60);
        if(strlen($minutes)==1) $minutes = '0'.$minutes;
        $sec = $timedifference%60;
        if(strlen($sec)==1) $sec = '0'.$sec;
        return $hours.':'.$minutes.':'.$sec;
    }     
    
    /**
    * Class method DateDiffInDays
    * return difference betwen two dates in days
    * @param integer $date1 - first date as timestamp. Must be earlier than $date2
    * @param integer $date2 - second date as timestamp. Must be bigger than $date1
    * @return integer $retval- count of days
    * @author Igor Trokhymchuk  <ihor@seotm.com>
    * @version 1.0, 27.12.2010
    */    
    ...
    function DaysArray()
    {
      ...
      for ($i=1; $i<=31; $i++)
      {
         if ( strlen($i)==1 ) $arr[$i]='0'.$i;
         else $arr[$i]=$i;
      }
      return $arr;
    } //End of function showDaysArray()
